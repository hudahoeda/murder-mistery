...
import { Puzzle, PuzzleStep } from '@/lib/types/game';
...
export default function TrainScheduleDemoPage() {
  ...
  useEffect(() => {
    const trainPuzzle = (puzzlesData.puzzles as any as Puzzle[]).find(p => p.id === 'train-schedule-investigation');
    if (trainPuzzle) {
      setPuzzle(trainPuzzle);
      setCurrentStep(trainPuzzle.steps[0]);
      setStepIndex(0);
        ...
        {!puzzleCompleted ? (
          ...
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <p className="text-slate-300">You successfully analyzed the train schedule.</p>
            </CardContent>
          </Card>
        )}
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <div className="space-y-4 text-sm text-slate-300">
              <div>
                <strong className="text-amber-200">Step 1:</strong> Platform Identification from CCTV evidence. Uses multiple choice selection.
              </div>
              <div>
                <strong className="text-amber-200">Step 2:</strong> Schedule Analysis requires calculating delay time. Uses numeric input.
              </div>
              <div>
                <strong className="text-amber-200">Step 3:</strong> Passenger Manifest search to find correlated passengers. Uses a searchable list.
              </div>
              <div className="mt-4 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                <p className="text-green-200">
                  <strong>Validation:</strong> Each step is validated based on the `validation` object in `puzzles.json`.
                  The demo page now manages the state of the steps.
                </p>
              </div>
            </div>
          </CardContent>
